...
import { ArrowDownUp, ArrowRight, Check, Download, EyeOff, Plus, Trash2, UserPlus } from 'lucide-react'
import { createId, downloadCsv, nowIso, reviewResultsToCsv, selectEligibleSubmissionsForRound, selectEvaluationRoundProgress, selectEvaluationRounds, selectNextEvaluationRound, selectReviewerProgress, selectRoundAssignments, selectRoundResults, selectRoundSubmissionScore, useApp } from '../../core'
        ...
        <>
          ...
          <div className="sb-plan-layout">
            ...
            <main className="sb-round-main">
              {selectedRound && <><RoundEditor key={selectedRound.id} round={selectedRound} onOpenReviewer={onOpenReviewer} /><RoundInsights round={selectedRound} /></>}
            </main>
          </div>
        </>
      )}
...
function RoundInsights({ round }: { round: EvaluationRound }) {
  ...
  return <div>
    <section className="sb-assignment-section" aria-labelledby={`reviewer-progress-${round.id}`}><div className="sb-card__header"><div><h3 id={`reviewer-progress-${round.id}`}>Reviewer progress</h3><p>Completion grouped by reviewer for this round.</p></div></div><div className="sb-assignment-table">{reviewers.map((reviewer) => <div key={reviewer.reviewerEmail}><span><strong>{reviewer.reviewerName}</strong><small>{reviewer.reviewerEmail}</small></span><span>{reviewer.completed}/{reviewer.assigned} completed · {reviewer.abstained} abstained</span><b>{reviewer.percent}%</b></div>)}{reviewers.length === 0 && <p className="sb-muted">No reviewers assigned.</p>}</div></section>
    <section className="sb-assignment-section" aria-labelledby={`round-results-${round.id}`}><div className="sb-card__header"><div><h3 id={`round-results-${round.id}`}>Round results</h3><p>Weighted aggregate scores across completed reviews.</p></div><div><button className="sb-button" type="button" onClick={() => setDirection((value) => value === 'asc' ? 'desc' : 'asc')}><ArrowDownUp aria-hidden="true" />Score {direction === 'asc' ? 'ascending' : 'descending'}</button><button className="sb-button" type="button" onClick={() => downloadCsv(`${round.name.replace(/\W+/g, '-').toLowerCase()}-review-scores.csv`, reviewResultsToCsv(state, round.id))}><Download aria-hidden="true" />Export scores</button></div></div><div className="sb-assignment-table">{results.map((result) => <div key={result.submission.id}><span><strong>{result.submission.title}</strong><small>{result.submission.track} · {result.submission.format}</small></span><span>{result.reviewCount} completed review{result.reviewCount === 1 ? '' : 's'}</span><b>{result.aggregate?.toFixed(2) ?? '—'} / 5</b></div>)}{results.length === 0 && <p className="sb-muted">No assigned proposals.</p>}</div></section>
  </div>
}
...
function RoundEditor({ round, onOpenReviewer }: { round: EvaluationRound; onOpenReviewer?: EvaluationPlanManagerProps['onOpenReviewer'] }) {
  ...
  const [opensAt, setOpensAt] = useState(localDateTime(round.opensAt ?? nowIso()))
  ...
  function saveRound(event: FormEvent<HTMLFormElement>) {
    ...
    dispatch({ type: 'evaluation/round/upsert', round: { ...round, name: name.trim(), status, opensAt: new Date(opensAt).toISOString(), dueAt: new Date(dueAt).toISOString(), blind, instructions: instructions.trim(), rubric: rubric.map((criterion) => ({ ...criterion, label: criterion.label.trim(), options: criterion.type === 'select' ? (criterion.options ?? []).map((option) => option.trim()).filter(Boolean) : undefined })), filter: { ...round.filter, tracks, submissionStatuses }, updatedAt: nowIso() }, at: nowIso() })
  ...
  return (
    <div>
      ...
      <form className="sb-round-config sb-form" onSubmit={saveRound}>
        <div className="sb-form__row"><label>Round name<input required value={name} onChange={(event) => setName(event.target.value)} /></label><label>Status<select value={status} onChange={(event) => setStatus(event.target.value as EvaluationRoundStatus)}><option value="draft">Draft</option><option value="open">Open</option><option value="closed">Closed</option></select></label><label>Opens<input required type="datetime-local" value={opensAt} onChange={(event) => setOpensAt(event.target.value)} /></label><label>Closes<input required type="datetime-local" value={dueAt} onChange={(event) => setDueAt(event.target.value)} /></label></div>
        ...
        <section className="sb-rubric" aria-labelledby={`rubric-${round.id}`}><div className="sb-card__header"><div><h3 id={`rubric-${round.id}`}>Weighted rubric</h3><p>Weights are normalized automatically. Current total: {totalWeight}.</p></div><button className="sb-button" type="button" onClick={() => setRubric((items) => [...items, { id: createId('criterion'), label: 'New criterion', type: 'rating', required: true, weight: 10, maxScore: 5 }])}><Plus aria-hidden="true" />Criterion</button></div>{rubric.map((criterion) => <div className="sb-rubric-row" key={criterion.id}><label>Criterion<input required value={criterion.label} onChange={(event) => setRubric((items) => items.map((item) => item.id === criterion.id ? { ...item, label: event.target.value } : item))} /></label><label>Answer type<select value={criterion.type ?? 'rating'} onChange={(event) => setRubric((items) => items.map((item) => item.id === criterion.id ? { ...item, type: event.target.value as RubricCriterion['type'] } : item))}><option value="rating">Numeric rating</option><option value="select">Dropdown</option><option value="text">Free text</option></select></label><label>Weight<input required min="1" max="100" type="number" value={criterion.weight} onChange={(event) => setRubric((items) => items.map((item) => item.id === criterion.id ? { ...item, weight: Number(event.target.value) } : item))} /></label>{(criterion.type ?? 'rating') === 'rating' && <label>Scale<input required min="2" max="20" type="number" value={criterion.maxScore} onChange={(event) => setRubric((items) => items.map((item) => item.id === criterion.id ? { ...item, maxScore: Number(event.target.value) } : item))} /></label>}{criterion.type === 'select' && <label>Options<input required value={(criterion.options ?? []).join(', ')} onChange={(event) => setRubric((items) => items.map((item) => item.id === criterion.id ? { ...item, options: event.target.value.split(',') } : item))} placeholder="Strong accept, Accept, Reject" /></label>}<label className="sb-check"><input type="checkbox" checked={criterion.required !== false} onChange={(event) => setRubric((items) => items.map((item) => item.id === criterion.id ? { ...item, required: event.target.checked } : item))} />Required</label><button className="sb-icon-button sb-icon-button--danger" type="button" aria-label={`Delete ${criterion.label}`} onClick={() => setRubric((items) => items.filter((item) => item.id !== criterion.id))}><Trash2 aria-hidden="true" /></button></div>)}</section>
