...
- `GET /api/public/cfp/:workspaceId/:eventSlug/claim?token=...` atomically consumes a valid unexpired token once and returns `{ data: { claimed: true, workspaceId, eventId, eventSlug, portalRoute, portalUrl, expiresAt } }`, giving the client an authoritative clean portal redirect without exposing the raw session. Invalid, expired, replayed, wrong-workspace, and wrong-event tokens all return the same `401 CLAIM_INVALID_OR_EXPIRED` response.
...
On those speaker-only handlers, a valid event claim takes precedence over ambient hosting identity. This lets an organizer test an emailed speaker link in an already signed-in browser without accidentally opening the organizer's email identity; all non-speaker routes continue using the hosting identity normally. An invalid or expired claim never overrides hosting identity.
